  ...
  return (
    <section className="py-24 px-6 md:px-20 bg-[#0a0a0a]">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-white text-3xl font-black tracking-tight mb-12 text-center uppercase tracking-[0.2em]">
          Sectors We Dominate
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 stagger-children">
          {cats.map((c, i) => (
            <div
              key={i}
              className="flex flex-col items-center gap-6 p-10 glass-card rounded-[2.5rem] hover:bg-primary/5 transition-all group"
            >
              ...
              <p className="text-white text-[10px] font-black uppercase tracking-[0.2em] text-center">
                {c.name}
              </p>
            </div>
          ))}
